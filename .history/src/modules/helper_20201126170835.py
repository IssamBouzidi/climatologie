...
class Helper:
    ...
    def remove_rows_for_wrong_temperature(df):
        ...
        df_to_remove = df[df['AvgTemperature'] <= -99]

        for index, row in temp_df.iterrows():
            avg_temp = df[(df['City'] == city) & (df['Month'] == month)  & (df['Day'] == day) & (df['AvgTemperature'] >= -99)]
            temp_df['AvgTemperature'] = round(avg_temp.mean(), 2)
