...
class Helper:
    ...
    def load_data(year_begin=1995, year_end=2020):
        ...
        dataframe = pd.read_csv(os.path.join(Helper.RAW_DIR, 'temperatures.csv'), usecols=fields)
    ...
    def preprocessing(df_temperatures):
        ...
        return df_temperatures_temp_final
    ...
    def save_report(df, name_report):
        if not path.exists(os.path.join(RAPPORT_DIR, f'{name_report}.html')):
            prof = ProfileReport(df)
            prof.to_file(output_file=os.path.join(RAPPORT_DIR, f'{name_report}.html'))
        else:
            print('File exists !')
