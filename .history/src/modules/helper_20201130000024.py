...
class Helper:
    ...
    @staticmethod
    def delete_incomplete_year(array_cols, df):
        # calculer le nombre de jours pour chaque ville et année
        temp_df_count = df.groupby(array_cols).size()

        # reinitialiser les index
        temp_df_count = temp_df_count.reset_index()
        
        # ajouter colonne Count
        new_cols = list(array_cols.append('Count'))

        # renomer les colonnes
        temp_df_count.columns = new_cols
    ...
    def preprocessing(df_temperatures):
        ...
        ###
        # supprimer toute ville qui manque des dates
        ###
        temp_df = Helper.delete_incomplete_year(array_cols=['City','Year'], df=df_temperatures_copy)        
        ...
        ###
        df_temperatures_dropna = df_temperatures_temp_final.dropna()

        # supprimer les villes dont les années ne sont pas completes
        df_temp = Helper.delete_incomplete_year(array_cols=['City', 'Year'], df= df_temperatures_dropna)
        
        # merger le dataframe temporaire avec le dataframe des temperatures
        df_final = df_temperatures_dropna.merge(df_temp, how='right', right_on=['City','Year'], left_on=['City','Year'])
