...
class Helper:
    ...
    def save_report(df, name_report):
        if not path.exists(os.path.join(RAPPORT_DIR, 'rapport_initial.html')):
            prof = ProfileReport(df_temperatures)
            prof.to_file(output_file=os.path.join(RAPPORT_DIR, 'rapport_initial.html'))
        else:
            print('File exists !')
